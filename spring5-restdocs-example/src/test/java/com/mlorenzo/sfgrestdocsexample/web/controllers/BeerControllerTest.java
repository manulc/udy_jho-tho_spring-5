...
// Por defecto, esta anotación utiliza el esquema http, el host localhost y el puerto 8080 para documentar los clientes http como, por ejemplo, curl 
// Podemos modificar estos valores por defecto usando los atributos "uriScheme", "uriHost" y "uriPort" de esta anotación
...
class BeerControllerTest {
	...
	void getBeerByIdTest() throws Exception {
		...
		mockMvc.perform(get("/api/v1/beer/{beerId}", beer.getId())
				// Pasamos a la url de la petición http el parámetro "isCold" con valor "yes". Realmente, el método handler del controlador asociado a esta petición http no maneja ningún parámetro de la url, es decir,
				// se trata de un ejemplo para mostrar cómo debe documentarse parámetros de la url de una petición http con REST Docs
				.param("isCold","yes")
				...
				.andDo(document("v1/beer-get",
						...
						// Para documentar la respuesta de la petición http, debe documentarse todas las propiedades o campos de la entidad asociada a la respuesta, ya que, de no hacerlo, el test fallará
						responseFields(
	...
	void saveNewBeerTest() throws Exception {
		...
		mockMvc.perform(post("/api/v1/beer/")
				...
				.andDo(document("v1/beer-new",
						// Para documentar la petición de la petición http, debe documentarse todas las propiedades o campos de la entidad asociada a la petición, ya que, de no hacerlo, el test fallará.
						// Usamos el método "ignored" en aquellas propiedades que no queremos que aparezcan en la documentación generada. En este caso, aplicamos dicho método en aquellas propiedades cuyos valores son generados por
						// la propia aplicación y no son introducidos por el usuario o cliente.
						requestFields(
